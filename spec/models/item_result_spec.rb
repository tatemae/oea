...
describe Item do
  ...
  describe 'results summary' do
    ...
    it 'should return results with eid' do
      results = ItemResult.results_summary(scope_url: nil, eid: @eid, keyword: nil)
      expect(results[:submitted]).to include(@item_result)
    end

    it 'should return no results when no matching eid' do
      results = ItemResult.results_summary(scope_url: nil, eid: FactoryGirl.generate(:name), keyword: nil)
    ...
    it 'should return results with identifier' do
      results = ItemResult.results_summary(scope_url: nil, identifier: @item.identifier, keyword: nil)
    ...
    it 'should not return results with identifier' do
      ...
      results = ItemResult.results_summary(scope_url: nil, identifier: @item2.identifier, keyword: nil)
      results[:submitted].should_not include(@item_result)
    end

    it 'should return results with scope' do
      results = ItemResult.results_summary(scope_url: @item_result.referer, identifier: nil, keyword: nil)
      results[:submitted].should include(@item_result)
    end
  end
end